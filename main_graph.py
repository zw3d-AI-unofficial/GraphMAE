...
def pretrain(model, pooler, dataloaders, optimizer, max_epoch, device, scheduler, num_classes, lr_f, weight_decay_f, max_epoch_f, linear_prob=True, logger=None):
    ...
    for epoch in epoch_iter:
        ...
        for batch in train_loader:                #[Graph(num_nodes=647, num_edges=11287,
            ...
            feat = batch_g.ndata["x"]             #修改
            model.train()
            loss, loss_dict = model(batch_g, feat) #batch_g Graph(num_nodes=570, num_edges=9642,    feat torch.Size([570, 271])     
        ...
        if scheduler is not None:
            scheduler.step()
        epoch_iter.set_description(f"Epoch {epoch} | train_loss: {np.mean(loss_list):.4f}") #多个batch的loss的均值
...
def collate_fn(batch):
    ...
    batch_g = dgl.batch(graphs)          # 将多个图合并为一个大图，并保持每个图的独立性 Graph(num_nodes=647, num_edges=11287,
