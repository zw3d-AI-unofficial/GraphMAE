...
def node_classification_evaluation(model, graph, x, num_classes, lr_f, weight_decay_f, max_epoch_f, device, linear_prob=True, mute=False):
    model.eval()                                             #评估模式
    if linear_prob:                                          #线性探测逻辑
        with torch.no_grad():                                #禁止梯度计算，提高效率
            x = model.embed(graph.to(device), x.to(device))
            in_feat = x.shape[1]
        encoder = LogisticRegression(in_feat, num_classes)
    else:
        encoder = model.encoder                              #GAT
        encoder.reset_classifier(num_classes)                #nn.Linear

    num_finetune_params = [p.numel() for p in encoder.parameters() if  p.requires_grad] #计算 encoder 中所有需要训练的参数的数量。
    if not mute:
        print(f"num parameters for finetuning: {sum(num_finetune_params)}")             #如果 mute 为 False，则打印出需要微调的参数总数
    
    encoder.to(device)
    optimizer_f = create_optimizer("adam", encoder, lr_f, weight_decay_f) #创建一个 Adam 优化器，用于训练分类器 encoder。优化器的参数包括学习率 lr_f 和权重衰减 weight_decay_f。
...
"""使用线性探测方法进行半监督节点分类任务，并采用了早停策略来防止过拟合"""
def linear_probing_for_transductive_node_classiifcation(model, graph, feat, optimizer, max_epoch, device, mute=False):
    ...
    for epoch in epoch_iter:
        model.train()                             #训练模式
        out = model(graph, x)                     #torch.Size([2708, 7])
